...
*/
		AlertAdmins()
			if(container && !alertadmins)
				if(istype(container, /datum/NTSL_Compiler))
					var/datum/NTSL_Compiler/Compiler = container
					var/obj/machinery/Holder = Compiler.Holder
					var/message = "Potential crash-inducing NTSL script detected at device [Holder] ([Holder.x], [Holder.y], [Holder.z])."
