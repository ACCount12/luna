...
mob/living/parasite/meme/Life()
	...
	var/gain = 4
	if(dormant) gain *= 4 // dormant recovers points faster
...
mob/living/parasite/meme/proc/use_points(amount)
	...
	if(src.meme_points < amount)
		src << "<b>* You don't have enough points!</b>"
		return 0
...
mob/living/parasite/meme/verb/Paralyze()
	...
	set desc     = "Prevents your host for a while."

	if(!src.host) return

	var/mob/living/carbon/human/host = src.host
	if(host.weakened)
		usr << "\red Your host is already paralyzed.."
		return
	if(!use_points(250)) return

	host << "\red Your body feels numb..."
...
mob/living/parasite/meme/verb/Analgesic()
	...
	host.reagents.add_reagent("hyperzine", 30)
	host.reagents.add_reagent("synaptizine", 30)
	while(host.reagents.has_reagent("hyperzine") || host.reagents.has_reagent("synaptizine"))
		sleep(10)
