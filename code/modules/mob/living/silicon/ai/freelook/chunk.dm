...
/datum/camerachunk/proc/add(var/mob/camera/aiEye/ai)
	if(!ai.ai)
		return
...
/datum/camerachunk/proc/remove(var/mob/camera/aiEye/ai)
	if(!ai.ai)
		return
	ai.visibleCameraChunks -= src
	if(ai.ai.client)
...
/datum/camerachunk/New(loc, x, y, z)
	...
	for(var/obj/machinery/camera/c in range(CHUNK_SIZE, locate(src.x + (CHUNK_SIZE / 2), src.y + (CHUNK_SIZE / 2), src.z)))
		if(c.can_use())
			cameras += c

	for(var/turf/t in block(locate(src.x, src.y, src.z), locate(min(src.x + CHUNK_SIZE - 1, world.maxx), min(src.y + CHUNK_SIZE - 1, world.maxy), src.z)))
		turfs[t] = t
