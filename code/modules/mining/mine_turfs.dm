...
/turf/simulated/mineral/exposed
...
/turf/simulated/mineral/New()
	spawn(1)
		...
		if((istype(get_step(src, NORTH), /turf/simulated/floor)) || (istype(get_step(src, NORTH), /turf/space)) || (istype(get_step(src, NORTH), /turf/simulated/shuttle/floor)))
			...
			if (T)
				T.overlays += image('icons/turf/overlays.dmi', "rock_side_s", layer=6)
		if((istype(get_step(src, SOUTH), /turf/simulated/floor)) || (istype(get_step(src, SOUTH), /turf/space)) || (istype(get_step(src, SOUTH), /turf/simulated/shuttle/floor)))
			T = get_step(src, SOUTH)
			if (T)
				T.overlays += image('icons/turf/overlays.dmi', "rock_side_n", layer=6)
		if((istype(get_step(src, EAST), /turf/simulated/floor)) || (istype(get_step(src, EAST), /turf/space)) || (istype(get_step(src, EAST), /turf/simulated/shuttle/floor)))
			T = get_step(src, EAST)
			if (T)
				T.overlays += image('icons/turf/overlays.dmi', "rock_side_w", layer=6)
		if((istype(get_step(src, WEST), /turf/simulated/floor)) || (istype(get_step(src, WEST), /turf/space)) || (istype(get_step(src, WEST), /turf/simulated/shuttle/floor)))
			T = get_step(src, WEST)
			if (T)
				T.overlays += image('icons/turf/overlays.dmi', "rock_side_e", layer=6)
	...
	if(istype(W, /obj/item/weapon/pickaxe) || istype(W, /obj/item/weapon/weldingtool/plasmacutter))
		...
		if(istype(W, /obj/item/weapon/pickaxe))
			...
			if(do_after(user,W:digspeed))
				...
				gets_drilled()
		else if (W:remove_fuel(1))
			user << "\red You cut the rock."
			gets_drilled()
	else
...
/turf/simulated/mineral/Bumped(AM as mob|obj)
	...
	if(istype(AM,/mob/living/carbon/human))
		...
		if((istype(H.l_hand,/obj/item/weapon/pickaxe) || istype(H.l_hand, /obj/item/weapon/weldingtool/plasmacutter)) && (!H.hand))
			src.attackby(H.l_hand,H)
		else if((istype(H.r_hand,/obj/item/weapon/pickaxe) || istype(H.r_hand, /obj/item/weapon/weldingtool/plasmacutter)) && H.hand)
...
/turf/simulated/floor/plating/airless/asteroid/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/weapon/shovel)))
	...
	else if ((istype(W,/obj/item/weapon/pickaxe/drill)))
		var/turf/T = user.loc
		if (!( istype(T, /turf) ))
			return
	...
	else if ((istype(W,/obj/item/weapon/pickaxe/diamonddrill)) || (istype(W,/obj/item/weapon/pickaxe/borgdrill)))
		var/turf/T = user.loc
		if (!( istype(T, /turf) ))
			return
		...
		var/obj/item/weapon/satchel/ore/S = W
		for(var/obj/item/weapon/ore/G in src.contents)
...
/turf/simulated/floor/plating/airless/asteroid/proc/updateMineralOverlays()
	...
	if(istype(get_step(src, NORTH), /turf/simulated/mineral))
		src.overlays += image('icons/turf/overlays.dmi', "rock_side_n", layer=6)
	if(istype(get_step(src, SOUTH), /turf/simulated/mineral))
		src.overlays += image('icons/turf/overlays.dmi', "rock_side_s", layer=6)
	if(istype(get_step(src, EAST), /turf/simulated/mineral))
		src.overlays += image('icons/turf/overlays.dmi', "rock_side_e", layer=6)
	if(istype(get_step(src, WEST), /turf/simulated/mineral))
		src.overlays += image('icons/turf/overlays.dmi', "rock_side_w", layer=6)
