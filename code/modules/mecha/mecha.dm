...
/obj/mecha/verb/toggle_lights()
	...
	lights = !lights
	if(lights)	ul_SetLuminosity(luminosity + lights_power)
	else		ul_SetLuminosity(luminosity - lights_power)
...
/obj/mecha/proc/get_stats_part()
	...
	var/output = {"[report_internal_damage()]
						...
						<b>Cabin pressure: </b>[cabin_pressure>325 ? "<font color='red'>[cabin_pressure]</font>": cabin_pressure]kPa<br>
...
/obj/mecha/Topic(href, href_list)
	...
	if (href_list["change_name"])
		var/newname = strip_html_simple(input(occupant,"Choose new exosuit name","Rename exosuit",initial(name)) as text, 26)
...
/datum/global_iterator/mecha_intertial_movement //inertial movement in space
	...
	process(var/obj/mecha/mecha as obj,direction)
		if(direction)
			if(mecha.check_for_support())
				src.stop()
			else
				step(mecha, direction)
		else
			src.stop()
		return
