...
/obj/machinery/mecha_part_fabricator
	...
	desc = "Nothing is being built now."
	density = 1
	anchored = 1
	req_access = list()
...
/obj/machinery/mecha_part_fabricator/New()
	...
	component_parts += new /obj/item/weapon/circuitboard/machine/mechfab(null)
...
*/
	if(!silent)
		...
		src.updateUsrDialog()
		sleep(10) //only sleep if called by user
	for(var/obj/machinery/computer/rdconsole/RDC in get_area(src))
		if(!RDC.sync) continue
