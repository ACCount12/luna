...
datum
	reagents
		...
		proc
				...
				do
					...
					for(var/datum/reagent/R in reagent_list) // Usually a small list
						for(var/reaction in chemical_reactions_list[R.id]) // Was a big list but now it should be smaller since we filtered it with our reagent id
							...
							if(!C.required_container) matching_container = 1

							else if(my_atom.type == C.required_container) matching_container = 1
							...
							if(!C.required_other)
								...
								if(istype(my_atom, /obj/item/weapon/slime_extract))
									var/obj/item/weapon/slime_extract/M = my_atom
									if(M.Uses > 0) matching_other = 1  // added a limit to slime cores -- Muskets requested this
			...
			remove_reagent(var/reagent, var/amount, var/safety = 1)
				if(!isnum(amount)) return 1
