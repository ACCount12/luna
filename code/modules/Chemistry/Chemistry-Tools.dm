...
/obj/item/weapon/grenade/chem_grenade/attack_self(mob/user)
	if(stage == READY &&  !active)
		...
		if(nadeassembly)
			...
			user << "<span class='warning'>You prime [src]! [det_time / 10] second\s!</span>"
		active = 1
...
/obj/item/weapon/grenade/chem_grenade/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/screwdriver))
		...
		if(nadeassembly)
			...
			new /obj/item/device/assembly/igniter(get_turf(src))
		if(beakers.len)
...
/obj/item/weapon/grenade/chem_grenade/on_found(mob/finder)
	if(nadeassembly)
		return nadeassembly.on_found(finder)
...
/obj/item/weapon/grenade/chem_grenade/large/prime()
	...
	var/has_reagents = 0
	var/obj/item/weapon/slime_extract/valid_core = null

	for(var/obj/item/weapon/reagent_containers/glass/G in beakers)
		if(!istype(G)) continue
		if(G.reagents.total_volume) has_reagents = 1
	for(var/obj/item/weapon/slime_extract/E in beakers)
...
/obj/item/weapon/grenade/chem_grenade/large/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/slime_extract) && stage == WIRED)
		user << "<span class='notice'>You add [I] to the assembly.</span>"
		user.drop_item()
		I.loc = src
		beakers += I
...
/obj/item/weapon/grenade/chem_grenade/metalfoam
	...
	New()
		..()
		//nadeassembly = new /obj/item/device/assembly_holder/time_ignite(src)
