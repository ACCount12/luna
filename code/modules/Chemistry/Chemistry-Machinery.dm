...
/obj/machinery/chem_dispenser
	...
	"sodium","aluminium","silicon","phosphorus","sulfur","chlorine","potassium","iron","copper","mercury","radium","water","ethanol","sugar","sacid"/*,"plasma"*/)
...
/obj/machinery/chem_dispenser/proc/recharge()
	...
	if(energy != oldenergy) use_power(1000) // This thing uses up alot of power (this is still low as shit for creating reagents from thin air)

/obj/machinery/chem_dispenser/power_change()
	if(powered())
		stat &= ~NOPOWER
	else
		spawn(rand(0, 15)) stat |= NOPOWER
...
/obj/machinery/chem_dispenser/Topic(href, href_list)
	...
	if (href_list["amtc"])
		...
		if (num) amount = text2num(num)

	if (href_list["amt"]) amount = text2num(href_list["amt"])

	if (href_list["dispense"] && energy && beaker)
		var/datum/reagents/R = beaker.reagents
		var/space = R.maximum_volume - R.total_volume

		R.add_reagent(href_list["dispense"], min(amount, energy * 20, space))
		energy = max(energy - min(amount, energy * 20, space) / 10, 0)
...
/obj/machinery/chem_dispenser/attackby(var/obj/item/weapon/reagent_containers/glass/B as obj, var/mob/user as mob)
	...
	if(!istype(B, /obj/item/weapon/reagent_containers/glass)) return
	if(beaker)
...
/obj/machinery/chem_dispenser/attack_hand(mob/user as mob)
	...
	if (beaker)
		var/datum/reagents/R = beaker.reagents
		dat += "\[H[R.total_volume]/[R.maximum_volume]\] \[<A href='?src=\ref[src];eject=1'>Eject</A>\]<BR><BR>"
		for(var/re in dispensable_reagents)
			for(var/da in typesof(/datum/reagent) - /datum/reagent)
				var/datum/reagent/temp = new da()
				if(temp.id == re)
					dat += "[temp.name]   <A href='?src=\ref[src];dispense=[temp.id]'>\[Dispense\]</A><BR>"
	user << browse("<TITLE>Chemical Dispenser</TITLE>Chemical dispenser menu:<BR>\[E[energy]\] \[<A href='?src=\ref[src];amtc=1'>A[amount]</A>\] \[<A href='?src=\ref[src];amt=5'>5</A>\] \[<A href='?src=\ref[src];amt=10'>10</A>\] \[<A href='?src=\ref[src];amt=30'>30</A>\] \[<A href='?src=\ref[src];amt=50'>50</A>\] \[<A href='?src=\ref[src];amt=100'>100</A>\]<BR>[dat]", "window=chem_dispenser")
...
/obj/machinery/chem_master/attackby(var/obj/item/weapon/reagent_containers/B as obj, var/mob/user as mob)
	...
	if(!istype(B, /obj/item/weapon/reagent_containers/glass) && !istype(B,/obj/item/weapon/reagent_containers/syringe)) return
...
/obj/machinery/chem_master/Topic(href, href_list)
	...
	if(beaker)
		...
		if (href_list["analyze"])
			...
			if(loaded_pill_bottle && loaded_pill_bottle.contents.len < 7)
...
/obj/machinery/chem_master/attack_hand(mob/user as mob)
	...
	if(!beaker)
		...
		if(src.loaded_pill_bottle)
			dat += "<A href='?src=\ref[src];ejectp=1'>Eject Pill Bottle \[[loaded_pill_bottle.contents.len]/7\]</A><BR><BR>"
		else
...
/obj/machinery/computer/pandemic/Topic(href, href_list)
	...
	if (href_list["create_vaccine"])
		...
		var/new_name = stripped_input(usr, "Name the Disease", "New Name", "", 24)
		if(..())
			return
...
/obj/machinery/reagentgrinder
	...
	var/list/blend_items = list (
		...
		/obj/item/weapon/coin/plasma = list("plasma" = 5),
		/obj/item/weapon/coin/uranium = list("uranium" = 5),
		/obj/item/weapon/coin/clown = list("banana" = 5),
		/obj/item/weapon/coin/silver = list("silver" = 5),
		/obj/item/weapon/coin/gold = list("gold" = 5),
...
/obj/machinery/reagentgrinder/proc/get_allowed_by_id(var/obj/item/weapon/O)
	for (var/i in blend_items)
		if (istype(O, i))
