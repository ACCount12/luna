/obj/machinery/artifact_device/harvester
	...
	var/rating = 1 // Energy collected per tick.

/obj/machinery/artifact_device/harvester/RefreshParts()
	..()
	rating = 0.2
	for(var/obj/item/weapon/stock_parts/capacitor/SM in component_parts)
		rating += SM.rating * SM.rating * 0.2

	for(var/obj/item/weapon/stock_parts/manipulator/M in component_parts)
		rating += M.rating * M.rating * 0.1

	for(var/obj/item/weapon/stock_parts/micro_laser/ML in component_parts)
		rating += ML.rating * ML.rating * 0.1
...
/obj/machinery/artifact_device/harvester/attackby(var/obj/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/weapon/screwdriver) && !harvesting)
		if (!open)
			open = 1
		...
		del(src)
		return 1
	else if(istype(O,/obj/item/weapon/anobattery) && !open)
		if(!inserted_battery)
			user << "\blue You insert [O] into [src]."
			user.drop_item()
			O.loc = src
			src.inserted_battery = O
			updateDialog()
		else
...
/obj/machinery/artifact_device/harvester/process()
	if(stat & (NOPOWER|BROKEN))
		return

	if(harvesting > 0)
		//gain a bit of charge
		inserted_battery.stored_charge += rating
		...
		//dump some charge
		inserted_battery.stored_charge -= rating * 4
...
/obj/machinery/artifact_device/harvester/Topic(href, href_list)
	if(..())
		return
	if(open)
		return
	...
	if (href_list["harvest"])
		...
		for(var/obj/machinery/artifact/art in locate(src.x + 1, src.y, src.z))
			articount++
			analysed = art
		...
		for(var/obj/O in locate(src.x + 1, src.y, src.z))
			...
			if(!istype(O, /obj/machinery/artifact))
				mundane++
				break
		for(var/mob/O in locate(src.x + 1, src.y, src.z))
			if(O.invisibility)
				continue
			mundane++
			break
