...
/obj/item/device/radio/talk_into(mob/living/M as mob, message, channel)
	...
	if(0 /*GLOBAL_RADIO_TYPE == 1*/) // NEW RADIO SYSTEMS: By Doohl
/*
		/* Quick introduction:
	  ...
	  /* ###### Intercoms and station-bounced radios ###### */
		...
		var/mob_name = M.GetVoice()
		var/mob_voice_name = M.voice_name
		var/voice_message = M.voice_message
		...
		if (length(heard_masked) || length(heard_normal) || length(heard_voice) || length(heard_garbled))
			...
			for(var/datum/signal_scrambler/scrambler in radio_controller.scramblers) // Simulate a radio signal transmission for scramblers. Hacky!
				if(!(display_freq in scrambler.freqs))
					continue

				if(scrambler.filter != RADIO_CHAT)
					continue

				if(scrambler.headsets_only && !istype(src, /obj/item/device/radio/headset))
					continue
				...
				signal.data = list(
						"message" = message, // the actual sent message
						"vmessage" = voice_message,
						"name" = mob_name,	// the mob's display name
						"vname" = mob_voice_name,
						"job" = eqjobname,		// the mob's job
						)

				scrambler.process_signal(signal) // Actually scramble it!

				message = signal.data["message"]
				voice_message = signal.data["vmessage"]
				mob_name = signal.data["name"]
				mob_voice_name = signal.data["vname"]
				eqjobname = signal.data["job"]
			...
			if (length((heard_normal + heard_masked)))
				var/rendered = "[part_a][mob_name][part_b][quotedmsg][part_c]"

				for (var/mob/R in (heard_normal + heard_masked))
					if(istype(R, /mob/living/silicon/ai))
						R.show_message("[part_a]<a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[mob_name] ([eqjobname]) </a>[part_b][quotedmsg][part_c]", 2)
					else
			...
			if (length(heard_voice))
				...
				for (var/mob/R in heard_voice)
					if(istype(R, /mob/living/silicon/ai))
						R.show_message("[part_a]<a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[mob_voice_name] ([eqjobname]) </a>[part_b][M.voice_message][part_c]", 2)
					else
						R.show_message(rendered, 2)
					for(var/obj/item/weapon/implant/imp in R)
						imp.hear(message,M)
			...
			if (length(heard_garbled))
				...
				for (var/mob/R in heard_voice)
					if(istype(R, /mob/living/silicon/ai))
						R.show_message("[part_a]<a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[mob_voice_name]</a>[part_b][quotedmsg][part_c]", 2)
					else
						R.show_message(rendered, 2)
					for(var/obj/item/weapon/implant/imp in R)
						imp.hear(message,M)

		for(var/obj/item/device/assembly/voice/radio/R in world)
			R.hear_talk(null, message, null, null, display_freq)
