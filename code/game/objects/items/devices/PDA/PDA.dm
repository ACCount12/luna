	...
	if (!owner)
		...
		switch (mode)
			if (0)
				...
				dat += "<br>Station Time: [round(world.time / 36000)+12]:[(world.time / 600 % 60) < 10 ? add_zero(world.time / 600 % 60, 1) : world.time / 600 % 60]"//:[world.time / 100 % 6][world.time / 100 % 10]"
				...
				if (cartridge)
					...
					if(istype(cartridge.radio, /obj/item/device/radio/integrated/beepsky))
				...
				if (cartridge)
					...
					if (istype(cartridge.radio, /obj/item/device/radio/integrated/signal))
						...
						if(!isnull(P))
							if (!P.toff && cartridge:shock_charges > 0)
								...
								if (!isnull(P.cartridge))
									...
									difficulty += P.cartridge.access_clown * 2
									difficulty += P.cartridge.access_janitor
									difficulty += P.cartridge.access_manifest

								if (prob(difficulty * 5) || P.uplink)
									U.show_message("\red An error flashes on your [src].", 1)
								else if (prob(difficulty * 2) && !uplink)
