...
/obj/effect/effect/chem_smoke/HasEntered(mob/living/carbon/M as mob )
	..()
	if(prob(90)) reagents.reaction(M)
	return
...
/datum/effect/effect/system/chem_smoke_spread
	...
	start()
		...
		var/mix_color = mix_color_from_reagents(chemholder.reagents.reagent_list)
		...
		for(i=0, i<src.number, i++)
			...
			spawn(0)
				...
				if(mix_color)
					smoke.icon += mix_color // give the smoke color, if it has any to begin with
				else
	...
	start()
		if (amount <= 2)
			...
			if (round(amount/12) > 0)
				devastation = min (6, devastation + round(amount/12))

			if (round(amount/6) > 0)
				heavy = min (9, heavy + round(amount/6))

			if (round(amount/3) > 0)
				light = min (12, light + round(amount/3))
